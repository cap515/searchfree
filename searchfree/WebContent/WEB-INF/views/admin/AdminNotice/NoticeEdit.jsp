					...
					<div class="container-fluid">
						...
						<div class="card card-body shadow m-2 border-left-primary">
							<form id="updateForm" role="form"
								action="<%=request.getContextPath()%>/admin/AdminNotice/update?no=<%=notice.getNoticeNo() %>"
								method="post">
								<br style="clear: both">
								<!-- <h4 class="mt-1 mb-4">게시글 수정 등록</h4> -->
								<div class="form-group mb-2">
									<select class="mr-2 mb-2" id="stNotice" name="stNotice">
										<option selected disabled hidden>게시판 선택</option>
										<option id="selectNotice" value="1">공지사항</option>
										<option id="selectQna" value="2">자주 묻는 질문</option>
									</select> <select class="mb-2" id="noticeStatus" name="noticeStatus">
										<option selected disabled hidden>게시 상태 선택</option>
										<option id="select-post" value="N">정상</option>
										<option id="select-hide" value="Y">숨김</option>
									</select>

									<script>
										$(function(){
											<% if(notice.getNoticeCode()==1){%>
													$("#selectNotice").prop("selected",true);
												<%}else if ((notice.getNoticeCode()==2)){%>
													$("#selectQna").prop("selected",true);
												<%}%>
												<% if(notice.getNoticeDeleteYN()=='N'){%>
												$("#select-post").prop("selected",true);
											<%}else if ((notice.getNoticeDeleteYN()=='Y')){%>
												$("#select-hide").prop("selected",true);
											<%}%>
										});
									</script>

									<input type="text" class="form-control mb-2" id="input-title"
										name="title" value="<%=notice.getNoticeTitle() %>" required>
								</div>
								<div class="form-group">
									<textarea id="summernote" name="content" class="form-control"><%=notice.getNoticeContent() %></textarea>
									<script>
								...
								<div class="float-right p-3">
									<button type="submit" class="btn btn-outline-primary m-1">수정</button>
									<a class="btn btn-outline-danger m-1"
										href="NoticeDetail?no=<%=notice.getNoticeNo() %>">취소</a>
								</div>
							</form>
						</div>
					</div>
